...
PlayTrickOrTreat = function(pumpkinId)
    ...
    if trickOrTreat < 0.5 then
        ...
        ShowNotification(Config.Translations.trick_notify..selected)
        dict[selected](pumpkinId)
    else
        local selected, dict = SelectRandomTrickOrTreat("Treats")

        ShowNotification(Config.Translations.treat_notify..selected)
        dict[selected](pumpkinId)
    end
end
...
SpawnPumpkin = function(id, coords, w)
    if not CreatedPumpkins[id] then
        CreatedPumpkins[id] = CreateObject(PickRandomPumpkinModel(), coords, false)
